...
const onomap_class = {
   ...
   convertToTimestamp: function (dateString) {
    ...
    updateHexOverlay: function (evt) {
        ...
        if (pos) {
            pos = pos.wrap();
            const gPos = proj4('EPSG:3857', [pos.lng, pos.lat]);
            const hPos = this.meterToHex(gPos[0], gPos[1]);
            if (this.lastDrawnHex.q != hPos.q || this.lastDrawnHex.r != hPos.r) {
                ...
                const center = this.hex_to_meter(hPos);
                const vertices = [];
                for (i = 0; i < 6; i++) {
                    const v = this.hex_corner(center, this.size, i);

                    const vGPos = proj4('EPSG:3857', 'EPSG:4326', [v.x, v.y]);
    ...
    setAutoTrack: function (newValue) {
        ...
        const thisgetHistory = L.Util.bind(this.getHistory, this);
        if (newValue) {
    ...
    showHistory: function (data) {
        ...
        if (data instanceof Array) {
            ...
            const histoDiv = document.getElementById('measures_log');
            const lang = $('#time_lang')[0].attributes.lang.value;
            moment.locale(lang);
            const onsite = $('input[name=tz-option]:checked')[0].attributes.onsite;
            ...
            for (let i = 0; i < data.length; i++) {
                ...
                const location_title = `${row.name_3}, ${row.name_1}, ${row.country}`;
                let country_iso = '';
    ...
    goToHistory: function (historyId) {
        const row = this.data_histo[historyId];
        if (row.bounds.type === 'Point') {
            const bounds = row.bounds.coordinates;
            ...
            const bounds = row.bounds.coordinates[0];
            if (!this.options["doAutoFly"]) {
                map.fitBounds([
        ...
        const startFlag = row.start.coordinates;
        const stopFlag = row.stop.coordinates;
    ...
    flyToLastMeasure: function () {
        ...
        const url = this.getFeatureInfoUrl('groovy:nc_last_measures'),
            doFlyToLastMeasure = L.Util.bind(this.doFlyToLastMeasure, this);
        const _this = this;
        const postData = this.getHistoryContent(this.partyData != null ? this.partyData.pk_party : "");
        $.ajax({
            type: 'POST',
            crossDomain: true,
            data: postData,
    ...
    postDumpArea: function (envelope, exportTracks, exportMeasures, exportAreas,exportRaw, fromEpoch, toEpoch) {
        ...
        $.ajax({
            ...
            url: url,
            success: function (data, status, xhr) {
                $("#extractFormArea").html(`<p>Dump in progress, download link will be updated in the following web page: </p><a href="https://data.noise-planet.org/custom_dump/${data}" target="_blank">Click here to open</a>`);
            },
            error: function (xhr, status, error) {
    ...
    getFeatureInfo: function (evt) {
        ...
        const postData = this.getFeatureInfoContent(evt.latlng);
        const _this = this;
        $.ajax({
            type: 'POST',
            crossDomain: true,
     ...
     xmlns:gml="http://www.opengis.net/gml" xmlns:ogc="http://www.opengis.net/ogc" 
      ...
      <wps:DataInputs>
        ...
        <wps:Input>
          ...
          <wps:Data>
            <wps:LiteralData>${exportTracks}</wps:LiteralData>
          </wps:Data>
        </wps:Input>
        <wps:Input>
          <ows:Identifier>exportMeasures</ows:Identifier>
          <wps:Data>
            <wps:LiteralData>${exportMeasures}</wps:LiteralData>
          </wps:Data>
        </wps:Input>
        <wps:Input>
          <ows:Identifier>exportAreas</ows:Identifier>
          <wps:Data>
            <wps:LiteralData>${exportAreas}</wps:LiteralData>
          </wps:Data>
        </wps:Input>
        <wps:Input>
  ...
  <wps:ResponseForm> 
    ...
    getFeatureInfoContent: function (latlng) {
        ...
        if (latlng) {
            const gPos = proj4('EPSG:3857', [latlng.lng, latlng.lat]);
            let partyId = null;
            ...
            const qIndex = hPos.q;
            const rIndex = hPos.r;
            return `<?xml version="1.0" encoding="UTF-8"?><wps:Execute version="1.0.0" service="WPS"
    ...
    loadHourlyGraphData: function (canvas_period, data) {
        const labels = ["0h", "1h", "2h", "3h", "4h", "5h", "6h", "7h", "8h", "9h",
        ...
        const graphsLabels = {'la50': 'LA50', 'laeq': 'LAeq'};
        for (const graphCat in graphs) {
            const graphEl = graphs[graphCat];
            if (graphEl != null) {
                const dataset = [];
                const colorset = [];
                for (let hour = 0; hour < 24; hour++) {
                    const hourDict = data[hour];
                    if (hourDict != null) {
                ...
                const chartIdentifier = `${canvas_period}_${graphCat}`;
    ...
    getColor: function (level) {
        ...
        const levels = Object.keys(this.COLOR_RAMP);
    ...
    loadUserRanksTable: function (rankDiv, users) {
        if (users instanceof Array && rankDiv != null) {
            let html_cont = `<table class="table table-striped"><thead><tr><th>${Transifex.live.translateText("Rank")}</th><th>${Transifex.live.translateText("Device Id")}</th><th>${Transifex.live.translateText("Number of measurements")}</th><th>${Transifex.live.translateText("Total duration")}</th></tr></thead><tbody>`;
    ...
    <p class='attribute_label'>Measure duration:</p><i class='fa fa-hourglass' aria-hidden='true'></i> ${content["measure_count"] ? Math.round(content["measure_count"]) + " seconds" : "None"}`;
        ...
        if (typeof content !== 'undefined') {
            ...
            if (typeof weekData !== 'undefined') {
                ...
                const loadHourlyGraphDataWorking_day = L.Util.bind(this.loadHourlyGraphData, this, 'working_day', weekData);
                setTimeout(loadHourlyGraphDataWorking_day, 50);
                const loadHourlyGraphDataSaturday = L.Util.bind(this.loadHourlyGraphData, this, 'saturday', saturdayData);
                setTimeout(loadHourlyGraphDataSaturday, 50);
                const loadHourlyGraphDataSunday = L.Util.bind(this.loadHourlyGraphData, this, 'sunday', sundayData);
                setTimeout(loadHourlyGraphDataSunday, 50);
            }
        }
        // Load tags
        if (typeof content !== 'undefined' && "tags" in content) {
            for (let idTag = 0; idTag < content["tags"].length; idTag++) {
